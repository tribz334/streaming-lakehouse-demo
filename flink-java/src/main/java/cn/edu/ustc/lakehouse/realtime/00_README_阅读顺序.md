...
读取 MySQL `order_detail` CDC，筛选支付成功的商品订单并执行订单生命周期去重。订单只携带用户、商品和交易金额，广告归属由点击流确定。
...
读取 MySQL `bill_detail` CDC，产出 `AdBill` 计费事实；随后由 `DwdAdBill` 转成 DWS 公共事实格式，为实时指标提供权威广告消耗。
