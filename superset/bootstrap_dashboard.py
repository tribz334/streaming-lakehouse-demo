...
def timeseries_chart(datasource, name, metric, y_axis_title):
    return {"slice_name": name, "viz_type": "echarts_timeseries_line", "params": {
        "datasource": f"{datasource.id}__table", "viz_type": "echarts_timeseries_line",
        "x_axis": "dt", "time_grain_sqla": "P1D", "time_range": "No filter",
        "metrics": [metric], "groupby": [], "adhoc_filters": [], "row_limit": 10000,
        "show_legend": False, "rich_tooltip": True, "truncate_metric": True,
        "x_axis_time_format": "smart_date", "y_axis_format": "SMART_NUMBER",
...
def layout_for(title, charts):
    ...
    cards = [chart for chart in charts if chart.viz_type == "big_number_total"]
    trends = [chart for chart in charts if chart.viz_type != "big_number_total"]
    rows = [("KPI", cards[start:start + 4]) for start in range(0, len(cards), 4)]
    rows += [("TREND", trends[start:start + 2]) for start in range(0, len(trends), 2)]
    for row_no, (row_kind, selected) in enumerate(rows, 1):
        row_id = f"ROW_{row_kind}_{row_no}"
...
def main():
    ...
    with app.app_context():
        ...
        realtime = db.session.query(SqlaTable).filter_by(table_name="v_realtime_metric_latest", schema="ad_ads").one()
        offline = db.session.query(SqlaTable).filter_by(table_name="v_offline_metric", schema="ad_ads").one()
        realtime_specs = [
            metric_card(realtime, "总消耗", "total_cost", "最新 10 秒窗口 · 元"), metric_card(realtime, "闭环消耗", "total_closed_cost", "最新 10 秒窗口 · 元"),
            metric_card(realtime, "广告 GMV", "total_pay_order_gmv", "最新 10 秒窗口 · 元"), metric_card(realtime, "实时 ROAS", "realtime_roas", "GMV / 闭环消耗", ",.2f"),
            ...
            metric_card(realtime, "图文广告 GMV", "total_image_text_pay_order_gmv", "ad_type=3"), metric_card(realtime, "搜索广告 GMV", "total_search_pay_order_gmv", "placement_type=2"),
            metric_card(realtime, "开屏广告 GMV", "total_splash_pay_order_gmv", "placement_type=3"), metric_card(realtime, "信息流广告 GMV", "total_feed_pay_order_gmv", "placement_type=1"),
        ...
        offline_specs = [
            metric_card(offline, "离线总消耗", "total_cost", "日级 · 元"), metric_card(offline, "离线闭环消耗", "total_closed_cost", "日级 · 元"), metric_card(offline, "离线广告 GMV", "total_pay_order_gmv", "日级 · 元"),
            ...
            metric_card(offline, "离线图文广告 GMV", "total_image_text_pay_order_gmv", "ad_type=3"), metric_card(offline, "离线搜索广告 GMV", "total_search_pay_order_gmv", "placement_type=2"),
            metric_card(offline, "离线开屏广告 GMV", "total_splash_pay_order_gmv", "placement_type=3"), metric_card(offline, "离线信息流广告 GMV", "total_feed_pay_order_gmv", "placement_type=1"),
            ...
            timeseries_chart(offline, "Cost 日趋势", "total_cost", "Cost（元）"),
            timeseries_chart(offline, "GMV 日趋势", "total_pay_order_gmv", "GMV（元）"),
        ]
